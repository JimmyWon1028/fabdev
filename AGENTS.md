...
## 目前 Windows 0.1.24 Stable 基線（2026-09-09）

公開 Latest 為 Windows-first Stable App `0.1.24`／Agent Protocol `39`。Annotated Tag `v0.1.24` 固定在 Commit `21bd978`，Release ID `385364449` 只包含 Windows x64 App 與 fabDev Connect；macOS ARM64 尚未補入。此版本完成每 PHP 系列 2／4／8 Worker 設定、每 Site timeout 與 Proxy 預設 120 秒 timeout；Windows x64 Run `34326154074` 與 `34329431684` 通過，Repository Owner 已明確回報實機 Gate 通過並核准 Publish。
...
此版本最近一次完整本機驗證為 Desktop 91、Release 規則 19、Rust 286、macOS Helper 9 項測試通過，另有 7 項需外部環境的 Rust 測試維持 ignored；`pnpm lint`、Cargo workspace check 與 `git diff --check` 通過。Draft Run `34330967986` 已從固定 Tag 重新建置 Windows x64 Setup 與 Connect，建立 7 個 App-only Assets，macOS Job skipped。全部 Assets 已在 Publish 前後重新下載，GitHub digest、大小、總表、個別 checksum、App／Stable Manifest、NSIS 內容及 x64 PE 架構驗證通過；公開 Release 頁與 Latest Stable Manifest 可匿名存取，目前沒有殘留 Draft Release。
