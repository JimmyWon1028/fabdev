...
## 目前 0.1.28 Stable 基線（2026-09-25）

公開 Latest 為 Windows-first 後已補齊 macOS ARM64 的 Stable App `0.1.28`／Agent Protocol `40`，提供 Windows 11 x64 Community Installer、fabDev Connect 與同版 macOS ARM64 Community DMG。Annotated Tag `v0.1.28` 固定在 Commit `f3645d5d20cfceca53f6dba6ae5849afcf4cd321`，Tag Object `ea5094153412913c66478ed393c2e9dcb42627d2`，Release ID `396444623`；Windows 候選 Run `36111295612`、Repository Owner 實機 Gate 與 Windows-first Draft Run `36112452167` 均已通過。macOS DMG 從相同 Tag Commit 建置，四個 bundled Runtime Archive 與上一版已驗證 DMG 逐位元相同。9 個 App-only Assets 共 151,385,822 bytes，已從未登入公開 URL 重新下載並通過 GitHub digest、大小、逐位元比對、總表、三份個別 checksum 與 App／Stable Manifest 驗證；原有 Windows Asset ID、大小與 digest 均未改變。Windows Setup SHA-256 為 `fe274ed4112629642d0147c1868a3125b5996d8ed3b56fab6269533cec9eabf2`，Connect 為 `962f9a69ef5ef527b33865d7e78d2193c1383e7cc24bed4726dfee62c07b62c0`，macOS DMG 為 `071591bce033f455cc93ead82a2a6988d92c2428a67714c9297d17c69b2ee3d0`。Latest Manifest 同時且只列 Windows x64 與 macOS ARM64 Installer。此版未處理線上 Runtime Package／Catalog。
...
## 前版 0.1.26 Stable 基線（2026-09-20）

前版為 Windows-first 後已補齊 macOS ARM64 的 Stable App `0.1.26`／Agent Protocol `40`，提供 Windows x64 App、fabDev Connect 與 macOS ARM64 Community DMG。Annotated Tag `v0.1.26` 固定在 Commit `76d6c2d`，Release ID `392254189`；Windows 候選 Run `35475685656`、Repository Owner 實機 Gate 及 Draft Run `35476309050` 均通過，Repository Owner 已明確核准 Publish 與同版 macOS 補齊。此版本新增預設關閉的「啟動時顯示主控面板」設定、修正背景 Quit 短暫顯示主視窗或 Dock 圖示、將一般頁面左右留白縮減為原本一半、強化必要服務狀態與 macOS Nginx master 辨識，並完整支援 HTTP／HTTPS Proxy upstream。

此版本最近一次完整本機驗證為 Desktop 100、Release 規則 20、Rust 298、macOS Helper 9 項測試通過，另有 7 項需外部環境的 Rust 測試維持 ignored；`pnpm lint`、Cargo workspace check 與 `git diff --check` 通過。Release 現有 9 個 App-only Assets、共 150,349,524 bytes，已全部從未登入公開 URL 重新下載並通過 GitHub digest、大小、逐位元比對、總表、三份個別 checksum 與 App／Stable Manifest 驗證。Windows Setup SHA-256 為 `3d15ddbd9475cdd30758f01de33b54e02d212aa0888110a834dbf2acb59f5303`，Connect 為 `fed39c5930c32ed6ca48b4e9a6308553952ad43eb86648b4d119d578d1fff8dd`，既有 Asset ID、大小與 digest 均未改變。macOS DMG 由固定 Tag Commit `76d6c2d50561d29fcc95e00e27c17187731b757a` 建置，為 100,053,655 bytes，SHA-256 `16218fb8309b35767714222332314f57e8d3934b9933438e287ef8a8b9572ef2`；`hdiutil verify`、內外 checksum、ad-hoc codesign、App／CLI 版本、主要 ARM64 Binary 及逐位元重用的四個 bundled Runtime Archive 均通過。該版公開 Release 頁、當時的 Latest Stable Manifest 與兩平台 Installer 可匿名存取；目前沒有殘留 Draft Release。
