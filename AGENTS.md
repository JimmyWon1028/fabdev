...
## 目前 0.1.26 Stable 基線（2026-09-20）

公開 Latest 為 Windows-first 後已補齊 macOS ARM64 的 Stable App `0.1.26`／Agent Protocol `40`，提供 Windows x64 App、fabDev Connect 與 macOS ARM64 Community DMG。Annotated Tag `v0.1.26` 固定在 Commit `76d6c2d`，Release ID `392254189`；Windows 候選 Run `35475685656`、Repository Owner 實機 Gate 及 Draft Run `35476309050` 均通過，Repository Owner 已明確核准 Publish 與同版 macOS 補齊。此版本新增預設關閉的「啟動時顯示主控面板」設定、修正背景 Quit 短暫顯示主視窗或 Dock 圖示、將一般頁面左右留白縮減為原本一半、強化必要服務狀態與 macOS Nginx master 辨識，並完整支援 HTTP／HTTPS Proxy upstream。
