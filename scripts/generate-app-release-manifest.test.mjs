...
test('keeps the Draft Release workflow manual and unable to publish', async () => {
  ...
  assert.match(workflow, /Release scope must be all or windows/)
  ...
  assert.match(workflow, /build-macos:\n[\s\S]*?if: inputs\.release_scope == 'all'/)
  ...
  assert.match(workflow, /inputs\.release_scope == 'windows' \|\| needs\.build-macos\.result == 'success'/)
...
test('loads and verifies every Windows online PHP Runtime from the package manifest', async () => {
  ...
  assert.match(script, /ext\/php_\$gdExtension\.dll/)
  assert.match(script, /extension_loaded\('gd'\)/)
  assert.match(script, /function_exists\('imagecreatetruecolor'\)/)
