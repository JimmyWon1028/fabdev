...
case "$PHP_VERSION" in
  7.4.33)
    PHP_BUILD_PROFILE="${PHP_BUILD_PROFILE:-php-legacy-modern-clang}"
    ...
    ;;
  8.2.33)
    PHP_BUILD_PROFILE="${PHP_BUILD_PROFILE:-php-modern}"
...
if [[ "$PHP_BUILD_PROFILE" == "php-modern-external-imap" ]]; then
  mkdir -p "$imap_source"
