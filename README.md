...
`v0.1.25` is the current Latest Stable. It was published Windows-first and now provides the Windows x64 App, fabDev Connect, and the matching macOS ARM64 Community DMG. It keeps Site and Proxy edit dialogs open during drag selection, protects unsaved edits, keeps Site names and Proxy IDs on one line, adds privacy-safe Site diagnostics, migrates managed paths in existing PHP configuration, and prevents transient 502 responses while switching PHP pools.

The latest full local run passed 97 Desktop tests, 19 Release rule tests, 296 Rust tests, 9 macOS Helper tests, `pnpm lint`, Cargo workspace check, and `git diff --check`; 7 Rust tests that require an external environment remain ignored. Windows candidate Run [`34704072483`](https://github.com/JimmyWon1028/fabdev/actions/runs/34704072483), the Repository Owner real-machine Gate, and Draft Run [`34704822912`](https://github.com/JimmyWon1028/fabdev/actions/runs/34704822912) passed. Annotated Tag `v0.1.25` remains fixed at commit `3191e67`; Release ID `387633199` is published with 9 verified App-only assets and Agent Protocol `40`. The macOS DMG is 100,053,217 bytes with SHA-256 `118a89b6435eb05cf788fc7d8c58dd82ba1661c6ffc54828449e26aa1578fe51`; the existing Windows binaries were not rebuilt or replaced.
...
Public App downloads are available from [fabdev GitHub Releases](https://github.com/JimmyWon1028/fabdev/releases). `v0.1.25` provides the Windows x64 App, fabDev Connect, and macOS ARM64 Community DMG. Starting with `v0.1.21`, an App Release contains only the App Installer, fabDev Connect, App Manifest, and SHA-256 files. Runtime Catalogs and Packages are managed independently through [fabdev-runtimes Releases](https://github.com/JimmyWon1028/fabdev-runtimes/releases). See [`docs/PUBLIC_RELEASE_SPEC.md`](docs/PUBLIC_RELEASE_SPEC.md) for Stable versioning, Asset names, Manifests, SHA-256, Draft/Publish, same-version platform supplements, and rollback contracts. `pnpm run release:prepare -- ...` only prepares an existing App installer and generates its Manifest and checksums. It does not package or publish anything and rejects Runtime Package input. `.github/workflows/release-draft.yml` accepts only manual dual confirmation and an existing Tag, and can only create or supplement a Draft. Publishing Stable still requires separate, explicit approval from the Repository Owner.
