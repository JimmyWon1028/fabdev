...
`v0.1.24` is the current Latest Stable for Windows x64 and macOS ARM64. It improves Windows PHP FastCGI concurrency, adds configurable per-Site upstream response timeouts, and changes the default timeout for new Proxy connections to 120 seconds. Existing explicitly saved Proxy timeout values remain unchanged, and the macOS PHP-FPM process model is not altered.

The latest full local run passed 91 Desktop tests, 19 Release rule tests, 287 Rust tests, 9 macOS Helper tests, `pnpm lint`, Cargo workspace check, and `git diff --check`; 7 Rust tests that require external Runtime packages or network access remain ignored. Windows x64 Runs [`34326154074`](https://github.com/JimmyWon1028/fabdev/actions/runs/34326154074), [`34329431684`](https://github.com/JimmyWon1028/fabdev/actions/runs/34329431684), and [`34432390677`](https://github.com/JimmyWon1028/fabdev/actions/runs/34432390677) passed, and the Repository Owner reported the real-machine Gate as passed. Annotated Tag `v0.1.24` remains at results commit `21bd978`; after explicit same-version replacement authorization, the current macOS ARM64 DMG was built from commit `e3fa037` with macOS PHP 8.5.10 online installation support. Release ID `385364449` remains published with 9 verified App-only assets. The existing Windows asset IDs, sizes, and digests were preserved.
...
Public App downloads are available from [fabdev GitHub Releases](https://github.com/JimmyWon1028/fabdev/releases). `v0.1.24` was published Windows-first and now provides the Windows x64 App, fabDev Connect, and the macOS ARM64 Community DMG. The Tag remains at commit `21bd978`; the current macOS DMG was explicitly replaced with a build from commit `e3fa037` to support macOS PHP 8.5.10 online installation. Starting with `v0.1.21`, an App Release contains only the App Installer, fabDev Connect, App Manifest, and SHA-256 files. Runtime Catalogs and Packages are managed independently through [fabdev-runtimes Releases](https://github.com/JimmyWon1028/fabdev-runtimes/releases). See [`docs/PUBLIC_RELEASE_SPEC.md`](docs/PUBLIC_RELEASE_SPEC.md) for Stable versioning, Asset names, Manifests, SHA-256, Draft/Publish, same-version platform supplements, and rollback contracts. `pnpm run release:prepare -- ...` only prepares an existing App installer and generates its Manifest and checksums. It does not package or publish anything and rejects Runtime Package input. `.github/workflows/release-draft.yml` accepts only manual dual confirmation and an existing Tag, and can only create or supplement a Draft. Publishing Stable still requires separate, explicit approval from the Repository Owner.
...
The current Runtime Latest is `catalog-v4`. Its `fabdev-runtime-v2.json` has `catalogSequence=4`, requires App `0.1.21` or later and Agent Protocol `37` or later, and lists seven Windows x64 entries and five macOS ARM64 entries. The new macOS entry is PHP 8.5.10; the previous eleven entries remain unchanged. A Catalog update changes only the installable list and does not rebuild every Package.
