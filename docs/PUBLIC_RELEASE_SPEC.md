...
>
> 狀態：`v0.1.0` Draft 因 macOS 驗收阻擋問題不得 Publish；`v0.1.1` 建立公開發布基線；`v0.1.21` 完成 App-only 與 Runtime Distribution 分離；`v0.1.28` 是目前 Latest Stable，已依 Windows-first 順序先發布 Windows x64 App 與 fabDev Connect，現已補齊同版 macOS ARM64 Community DMG。
