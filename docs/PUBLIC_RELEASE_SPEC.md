...
>
> 狀態：`v0.1.0` Draft 因 macOS 驗收阻擋問題不得 Publish；`v0.1.1` 建立公開發布基線；`v0.1.21` 完成 App-only 與 Runtime Distribution 分離；`v0.1.23` 是目前 Windows-first Latest Stable，只包含 Windows x64 App 與 fabDev Connect，macOS ARM64 尚未補入
