...
- 2026-09-02：依 Repository Owner 明確授權，將既有 Windows-only `v0.1.20` Draft 補齊 macOS ARM64。GitHub Actions [Run 33630602638](https://github.com/JimmyWon1028/fabdev/actions/runs/33630602638) 與修正版 [Run 33636835829](https://github.com/JimmyWon1028/fabdev/actions/runs/33636835829) 的 macOS bundled／online Runtime、sidecar、完整測試、lint、Unsigned Community DMG 與 Artifact Jobs 均成功，Windows Jobs 均為 skipped；最後 Draft Gate 分別揭露更新 Notes 時未明確保留 Tag，以及續跑時仍固定要求 20 個既有 Assets，已由 Commit `437be40`、`633ea9f` 修正為保留 `tag_name=v0.1.20` 並可從 20／30 Assets 安全續跑。Release ID `381210149` 目前含 30 個跨平台 Assets、685,600,881 bytes，仍為 `draft=true`、`prerelease=false`、`published_at=null`。全部 Assets 重新下載後，`SHA256SUMS` 內 13 個主要檔案與 13 份個別 checksum 均通過；App／Stable Manifest 逐位元一致，包含 Windows x64 與 macOS ARM64 各一個 Installer；Runtime Catalog sequence 14、minimum App `0.1.20`，包含 Windows 6＋macOS 4 共 10 個 Runtime。`SHA256SUMS` SHA-256 為 `d27f5c2e2500520cefe1f7e608a7f05c3e252c9a5c4897c69d54ae30d848a832`，App／Stable Manifest 為 `36615138a917a63be41a56732919c0e9c0146a11776f9d326fc0f80c52ee4130`，Runtime Catalog 為 `71d248604dde5f572b36ec2224dc1429e62e6ae303ef653a9b9262533a6a5b73`，macOS DMG 為 `e4c48d0870e3a08eb4cfcf494a883baeaec92360283fed9ba81590ed94f7207e`，Windows Setup 維持 `0344df9ae72aa2dcb306510e137c069e3e213ca50deca9ebc28b4bd5b733fbc7`。下載後 DMG `hdiutil verify`、內部 28 筆 checksum、Desktop／CLI／Agent 0.1.20、Desktop／Agent／CLI ARM64 與 App 深度簽章均通過；App 維持 `Signature=adhoc`、`TeamIdentifier=not set`，未加入 Developer ID、notarization、stapling 或 Hardened Runtime。依既定規則未重跑 macOS 或 Windows 實機安裝／啟動／移除；Draft checklist 的資產與 Manifest 驗證已勾選，Publish 核准仍未勾選，公開 Latest 仍為 `v0.1.19`。
