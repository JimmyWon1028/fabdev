...
- 2026-09-13 Repository Owner 明確要求開始 `0.1.25` 進版打包程序並依協議先做 Windows x64。根目錄／Desktop `package.json`、Tauri 設定、Cargo workspace 與 `Cargo.lock` 內 13 個 fabDev 套件已同步為 `0.1.25`，Agent Protocol 為 `40`。本機完整 `pnpm test` 通過 Desktop 97、Release 規則 19、Rust 296 與 macOS Helper 9 項測試，另有 7 項外部環境測試維持 ignored；Cargo workspace check、`pnpm lint` 與 `git diff --check` 通過。候選 Commit `563e551ff98d82ae1a935be5c9c7fb8883c18dc6` 已推送；Windows x64 Run [`34704072483`](https://github.com/JimmyWon1028/fabdev/actions/runs/34704072483) 於 2026-09-13 00:03:31～00:09:56（Asia/Taipei，UTC+8）完成，格式、前端測試、Release 契約、Bundled Windows Runtimes、PHP FastCGI worker pool、MSVC workspace、fabDev Connect、Unsigned NSIS 與兩個 Artifact 上傳全數成功，Check Run annotations 為 0。Artifact `fabDev-Community-Windows-x64` ID `10300539683`、ZIP 49,503,640 bytes；`fabDev-Connect-Windows-x64` ID `10301148101`、ZIP 332,543 bytes，保留至 2026-12-12 00:03:28（Asia/Taipei，UTC+8）。Repository Owner 已明確回報此候選的 Windows `0.1.25` 實機 Gate 通過，並授權提交及推送 Gate 結果、建立 annotated Tag `v0.1.25`、重新打包 Windows x64 及建立 Windows-first Draft；不包含 Publish Stable、macOS 或線上 Runtime Package 授權。
...
- Runtime Catalog schema v2 以單調遞增 sequence 管理安裝列表；`catalog-v1` 保存 11 個既有 Package，`catalog-v2` 驗證可只移除 Node.js 20.20.2，`catalog-v3` 重用原 URL／大小／SHA-256 恢復，`catalog-v4` 保留原 11 項並新增 macOS PHP 8.5.10。
...
- 2026-09-10：Repository Owner 明確授權重新打包並替換 `v0.1.24` 的 macOS ARM64 Asset。建置使用隔離 detached worktree 與 Commit `e3fa037611300f4979a336812633d0663666cc5a`，加入 macOS PHP 8.5.10 線上安裝支援；Annotated Tag 仍固定在 Commit `21bd9787ed5ccb1dd5ff5900df1e1ce2c158b38e`。四個內建 Runtime Archive 與先前 `v0.1.24` DMG 逐位元一致，未重新編譯未變更的 bundled Runtime。Desktop 91、Release 規則 19、Rust 287、macOS Helper 9 項測試通過，另有 7 項需外部 Runtime／PHP／公開網路的 Rust 測試維持 ignored；`pnpm lint`、Cargo workspace check、Helper lint、`git diff --check` 及 Windows x64 Run [`34432390677`](https://github.com/JimmyWon1028/fabdev/actions/runs/34432390677) 通過。新 DMG 為 100,061,256 bytes，SHA-256 `877d9b94e62622118f45f52cf1dffa63120b3521dcaed293f2ab443164ce6150`；`hdiutil verify`、DMG 內外 checksum、App／CLI／Helper ad-hoc codesign 及 ARM64 架構均通過。DMG 內 Release Agent 已從公開 Catalog sequence 4 實際下載、驗證並安裝 PHP 8.5.10，PHP CLI 與主要模組載入正常。Release ID `385364449`、Release 發布時間、Tag Object 與 peeled Commit 均未改變；既有 Windows／Connect Asset ID、大小與 digest 均未改變。九個公開 Assets 共 150,295,248 bytes，已由未登入 URL 全部重新下載並與最終產物逐位元一致；個別與總 checksum、GitHub digest、Latest Manifest、DMG 映像均驗證通過。App／Stable Manifest SHA-256 為 `54a024d8a58290772a7256cd0f144fdfb5016d58d4fde636435b59a94a205a78`，固定 `publishedAt=2026-09-09T08:45:10Z` 且只列 Windows x64 與 macOS ARM64 Installer。
- 2026-09-10：`fabdev-runtimes` Commit `487aebf51a62dfc6330417483a8da09f9babd6f7` 已發布 Latest [`catalog-v4`](https://github.com/JimmyWon1028/fabdev-runtimes/releases/tag/catalog-v4)，Release ID `385979498`，發布時間為 2026-09-10 11:16:03（Asia/Taipei, UTC+8）。Catalog sequence 4 保留原有 11 個項目的 URL、大小與 SHA-256，只新增 macOS ARM64 PHP 8.5.10；Catalog 為 10,054 bytes、SHA-256 `537524b0d22a9facfb94b41f4a35d6ea29b50d54597d8746824b4386a899a226`，PHP Package 為 52,772,681 bytes、SHA-256 `becf06f1d74d63464c31bb05c5b2f3fadc2ce27fd4363dc4cbf92a4068d327e9`。三個公開 Assets 已驗證大小、GitHub digest、`SHA256SUMS` 及未登入下載；開發版與新 DMG 的 Release Agent 均完成 PHP 8.5.10 真實線上安裝。
...
- 2026-09-09：Repository Owner 明確授權進版與推送 CI；Commit `c66423bd93e878f1d3ada023a60f0716fc602bbf` 將根目錄／Desktop `package.json`、Tauri 設定與 Cargo workspace 四個正式版本來源，以及 `Cargo.lock` 內 13 個 fabDev workspace 套件同步為 `0.1.24`，Agent Protocol 維持 `39`。本機完整 `pnpm test` 通過 Desktop 91、Release 規則 19、Rust 286、macOS Helper 9 項測試，另有 7 項需外部 Runtime／網路環境的 Rust 測試維持 ignored；Cargo workspace check、`pnpm lint` 與 `git diff --check` 通過。Push 觸發的 Windows x64 Run [`34329431684`](https://github.com/JimmyWon1028/fabdev/actions/runs/34329431684) 在 7 分 33 秒內完成，格式、前端、Windows 發布契約、FastCGI worker、MSVC workspace、fabDev Connect、Unsigned NSIS 與兩個 Artifact 上傳全數成功。Artifact `fabDev-Community-Windows-x64` 與 `fabDev-Connect-Windows-x64` 分別為 49,475,987 與 332,547 bytes；本輪未建立 Tag、Draft 或 Publish，也未打包 macOS 或線上 Runtime Package。
- 2026-09-09：Repository Owner 明確授權提交 CI 結果文件、建立 `v0.1.24` Tag、重新打包 Windows x64 與建立 Draft。CI 結果文件 Commit `21bd9787ed5ccb1dd5ff5900df1e1ce2c158b38e` 已推送，Annotated Tag `v0.1.24` 固定在該 Commit。Draft workflow [Run `34330967986`](https://github.com/JimmyWon1028/fabdev/actions/runs/34330967986) 的 Request、Windows x64 建置與 Draft Jobs 全數成功，macOS Job 依指定 skipped；建立 Release ID `385364449`，狀態為 `draft=true`、`prerelease=false`、`published_at=null`。Draft 只有 7 個 App-only Assets，共 50,232,874 bytes，未含線上 Runtime Package、Catalog 或 macOS Asset；Windows Setup 為 49,480,938 bytes、SHA-256 `97fda654b0fcf6fe3159ec9c3fbd5dcc99778c445e89185add34fa7769f5452a`，Connect 為 749,568 bytes、SHA-256 `210d58934da7f17504885a06a5dfbd3ad981dda03f3bc5f1074a4cd8e6ef9792`，App／Stable Manifest SHA-256 為 `92ce9505242fffb4f8a08f8445aa35a4483af724025ebc89d2d49594e454a258`，`publishedAt` 預留為 `2026-09-09T08:45:10Z`。資產上傳後重新下載、checksum／Manifest／NSIS 靜態驗證與明確 Publish 授權仍未完成；公開 Latest 維持 `v0.1.23`。
- 2026-09-09：Repository Owner 明確核准 `v0.1.24` Windows-first Stable Publish。Release ID `385364449` 於 2026-09-09 17:20:48（Asia/Taipei, UTC+8）發布，狀態為 `draft=false`、`prerelease=false`，並成為 Latest Stable。全部 7 個 App-only Assets 已在 Publish 前後由 Draft／未登入公開 URL 重新下載，Asset ID、GitHub digest、大小與位元組均未改變；`SHA256SUMS`、兩份個別 checksum、App／Stable Manifest 逐位元一致性、App `0.1.24`、Protocol `39`、單一 Windows x64 Installer、214 項 NSIS 內容及 Desktop／Agent／Helper／Connect／PHP x64 PE 架構全數通過。公開 Release 頁回傳 HTTP 200，Latest Stable Manifest 與版本 Manifest 一致。Setup SHA-256 維持 `97fda654b0fcf6fe3159ec9c3fbd5dcc99778c445e89185add34fa7769f5452a`，Connect 維持 `210d58934da7f17504885a06a5dfbd3ad981dda03f3bc5f1074a4cd8e6ef9792`，Manifest 維持 `92ce9505242fffb4f8a08f8445aa35a4483af724025ebc89d2d49594e454a258`。遠端 Annotated Tag Object `022a2b8e89033dffd7eb946cdb53a1563428f602` 與 peeled Commit `21bd9787ed5ccb1dd5ff5900df1e1ce2c158b38e` 未移動；目前沒有殘留 Draft Release，未包含 macOS Asset 或線上 Runtime Package／Catalog。
...
- 2026-09-03：公開儲存庫 [`JimmyWon1028/fabdev-runtimes`](https://github.com/JimmyWon1028/fabdev-runtimes) 已接管選裝 Runtime。Latest [`catalog-v3`](https://github.com/JimmyWon1028/fabdev-runtimes/releases/tag/catalog-v3) 於 2026-09-03 14:11:54（Asia/Taipei, UTC+8）發布；`fabdev-runtime-v2.json` 為 9,235 bytes、SHA-256 `abfdd876bee9b59d828d74452c26f32e5a0720e63d698b35e497081c0bb92676`，`catalogSequence=3`、最低 App `0.1.21`、最低 Agent Protocol `37`、到期時間 `2027-03-02T23:59:59Z`，列出 Windows x64 7 項與 macOS ARM64 4 項。`catalog-v2` 移除 Node.js 20.20.2、`catalog-v3` 恢復的用戶端重新整理與安裝流程均由 Repository Owner 回報測試 OK；恢復只重用 `catalog-v1` 的 Package URL、大小及 SHA-256，沒有重新打包或上傳 Package。
...
- [x] `v0.1.21` 已完成 App-only Release 與 Runtime Distribution 分離；App Release 保留 9 個跨平台 App Assets，選裝 Runtime 由獨立 `fabdev-runtimes` 的 `catalog-v4` 提供，兩者不共用版號或發布週期。
...
- [x] 依 [`RUNTIME_ONLINE_UPDATE_SPEC.md`](RUNTIME_ONLINE_UPDATE_SPEC.md) 完成歷史 Runtime Catalog v1 與 PHP 8.4.24 兩平台 Side-by-side 線上安裝；Windows x64 已由 `v0.1.12` 完成 CI、封裝、Publish、匿名 Feed 與真實線上重裝，macOS ARM64 已由 `v0.1.13` 完成本機安裝／更新、公開 Catalog／Package 與檔案級驗收。`v0.1.21` 起改由 [`RUNTIME_CATALOG_V2_SPEC.md`](RUNTIME_CATALOG_V2_SPEC.md) 的獨立 `fabdev-runtimes` Latest Catalog 持續提供兩平台 PHP 8.4.24。
