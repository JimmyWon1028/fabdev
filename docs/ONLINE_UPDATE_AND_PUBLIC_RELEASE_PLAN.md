...
> 文件狀態：P0 公開下載基礎與 P1 App 內更新均已完成；`v0.1.3` 已完成 Stable Publish、匿名公開下載及封裝版線上更新驗收；P2.1～P2.3 Runtime Catalog、下載、UI 與 Side-by-side 安裝完成，下一階段為 P2.4 兩平台 Draft 驗收
...
- Runtime Catalog v1 規格、P2.1 Typed Model／產生器／Parser／Validator、P2.2 Agent Protocol 33／下載操作狀態，以及 P2.3 PHP Runtime UI／安裝已完成；見 [`RUNTIME_ONLINE_UPDATE_SPEC.md`](RUNTIME_ONLINE_UPDATE_SPEC.md)。
- 第一個目標為 PHP 8.4.24 macOS ARM64／Windows x64 Side-by-side 安裝，不自動切換 Site 或全域 PHP。
- Agent HTTPS 下載、`.part`、大小／SHA-256、操作狀態、取消／重試、安裝前重新驗證、固定 PHP 健康檢查、Side-by-side 安裝與失敗清理已完成；公開 Feed 及 macOS／Windows 封裝版端到端驗收留待 P2.4。
